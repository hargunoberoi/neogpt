...
def train(rank,world_size):
    ...
    train_loader = DataLoaderLite(B, T, rank, world_size, split="train", master_process=(rank == 0))
    val_loader = DataLoaderLite(B, T, rank, world_size, split="val", master_process=(rank == 0))                                    
    ...
    # always start from scratch (no restart logic)
        ...
        for iter in range(start_iter, max_iters):
            ...
            if rank == 0:
                if args.logging and run is not None:
                    ...
                    print(f"Training loss at step {iter}: {loss_accum:.4f}")
                    with open(os.path.join(logs_dir, "train_loss.txt"), "a") as f:
                        f.write(f"Step {iter}: {loss_accum:.4f}\n")
                else:
                    print(f"Step {iter}: train/loss={loss_accum:.4f}, train/lr={lr:.6f}, train/norm={norm:.4f}, tokens/sec={tokens_processed / dt:.2f}")
